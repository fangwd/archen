    ...
    for (const table of this.db.tables) {
      ...
      for (const column of table.columns) {
        ...
        if (column.references) {
          modelFields[table.name][column.references.table.name] = {
            type: modelTypes[column.references.table.name],
            resolve(object, args, context) {
              return context.loader.load(column.references, object[column.name]);
            },
          };
          whereFields[table.name][column.references.table.name] = {
          ...
          modelFields[table.name][fieldName] = {
            ...
            resolve(object, args, context) {
              if (args.where) {
                ...
                return context.loader.query(col.table.name, args);
              } else {
    ...
    for (const table of this.db.tables) {
      ...
      queryFields[name] = {
        ...
        resolve(_, args, context) {
          return context.loader.query(table.name, args);
        },
      };
      queryFields[table.name] = {
        ...
        resolve(_, args, context) {
          return new Promise(resolve => {
            context.loader.query(table.name, args).then(rows => {
              resolve(rows[0]);
            });
          });
        },
