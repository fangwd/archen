  ...
  update(table, args) {
    ...
    return new Promise(resolve => {
      ...
      query
        .update(args.data)
        .then(() => {
          me.query(table, { where: args.where }).then(rows => {
            console.log(rows);
            resolve(rows[0]);
          });
        })
...
class Builder {
  ...
  scope(name) {
    ...
    name = name || '*';
    return this.flat ? name : `t${this.number}.${name}`;
  }

  as(table) {
    table = table || this.table;
    return this.flat ? table.name : `${table.name} as t${this.number}`;
  }
