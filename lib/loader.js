...
class Loader {
  ...
  create(table, args) {
    return createRow(this.db, table, args.data);
    /*
  ...
  update(table, args) {
    ...
    return new Promise(resolve => {
      ...
      resolveFields(this.db, table, args.data).then(data => {
...
function resolveFields(db, table, args) {
  ...
  for (const key in args) {
    ...
    if (column) {
      ...
      if (!(column = table.relatedKey(key))) {
...
function createRow(db, table, args) {
  return new Promise(resolve => {
    resolveFields(db, table, args).then(data => {
      db(table.name)
        .insert(data)
        .then(id => {
          const todos = [];
          for (const key in args) {
            const column = table.relatedKey(key);
            if (column) {
              todos.push({ column, args: args[key] });
            }
          }
          ...
          if (nextKey >= keys.length) {
            ...
            if (key === 'connect') {
              ...
              for (const arg of args[key]) {
                query.orWhere(arg[Object.keys(arg)[0]]);
              }
              query.then(_resolve2);
            } else {
              if (key !== 'create') throw Error('Unknown operation: ' + key);
