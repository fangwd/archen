...
class Loader {
  ...
  query(table, args) {
    const query = buildQuery(this.db, table, args, 0);
    const me = this;
...
class Builder {
  ...
  buildWhere(args, level) {
    ...
    return function() {
      for (const key in args) {
        ...
        const column = me.table.column(name);
        if (column) {
          name = column._name;
        }
        ...
        switch (op) {
          ...
          case 'exists':
            ...
            break;
          case 'some':
            this.whereExists(me.buildQuery(name, value, level));
            break;
          case 'none':
            this.whereNotExists(me.buildQuery(name, value, level));
            break;
          default:
            if (!op) {
              if (name === 'AND') {
  ...
  buildQuery(name, args, level) {
    ...
    const args2 = transformWhere(column.table, args);
    builder.join(query, args2, builder.number);
...
function buildQuery(db, table, args) {
  ...
  if (args.where) {
    const where = transformWhere(table, args.where);
...
function updateRow(db, table, args) {
  ...
  const where = builder.buildWhere(transformWhere(table, args.where));
  return new Promise((resolve, reject) => {
