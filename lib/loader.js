      ...
      if (next >= todos.length) {
        ...
        if (Array.isArray(args)) {
          ...
          if (key === 'connect') {
            ...
            updateRow(db, column.references.table, args[key]).then(row => {
              result[column._name] = row[column.references.table.primaryKey.name];
              _resolve();
            });
          } else if (key === 'create') {
            ...
            throw Error('Unknown operation: ' + key);
          }
        }
      }
    }
